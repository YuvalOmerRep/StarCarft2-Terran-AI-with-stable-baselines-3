...
def run_single_game():
    ...
    print(run_game(maps.get("AbyssalReefLE"), [my_bot, computer], disable_fog=True))
...
def run_num_of_games_against_same_computer(num: int, difficulty, my_bot):
    ...
    results = []
    for i in range(num):
        res = run_game(maps.get("AbyssalReefLE"), [my_bot, computer], realtime=False, game_time_limit=GB.GAME_TIME_LIMIT, disable_fog=True)
        results.append(res)
